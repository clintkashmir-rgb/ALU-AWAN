  ...
  return (
    <div className={`space-y-3 p-4 border rounded-lg transition-all ${isExcluded ? 'bg-muted/30 opacity-50 grayscale' : 'bg-card shadow-sm'}`}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Input type="number" step="any" className="w-[90px] h-9" value={state.constant} onChange={(e) => setState({ ...state, constant: e.target.value })} placeholder="0" />
...
export default function FormulasPage() {
  ...
  React.useEffect(() => {
    if (sections && sections.length > 0 && !selectedSectionId) setSelectedSectionId(sections[0].id)
  ...
  React.useEffect(() => {
    if (currentSection) {
      const parseFormula = (f: string | undefined) => {
        ...
        return parts.length < 3 ? { variable: "None", operator: "+", constant: "0" } : { variable: parts[0], operator: parts[1], constant: parts[2] }
      }
  ...
  const handleSave = () => {
    ...
    const formatStr = (state: any) => state.variable === 'None' ? 'None' : `${state.variable} ${state.operator} ${state.constant || '0'}`
    updateDocumentNonBlocking(doc(firestore, "sections", selectedSectionId), {
  ...
  const handleDeleteFormula = (sectionId: string) => {
    ...
    updateDocumentNonBlocking(doc(firestore, "sections", sectionId), {
      top_formula: "None", bottom_formula: "None", side_formula: "None", updatedAt: new Date().toISOString()
    });
    toast({ title: "Logic Reset" });
  }

  const configuredSections = sections?.filter(s => 
    (s.top_formula && s.top_formula !== 'None') || (s.bottom_formula && s.bottom_formula !== 'None') || (s.side_formula && s.side_formula !== 'None')
  ) || []
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4 sticky top-0 bg-background/80 backdrop-blur-sm z-10">
          ...
          <h1 className="font-headline text-xl font-bold flex items-center gap-2 uppercase">
            <Calculator className="h-5 w-5 text-accent" /> Formula Builder
          </h1>
        </header>
        <main className="flex-1 p-4 md:p-6 space-y-8 max-w-4xl mx-auto pb-20">
          <Card className="border-none shadow-xl">
            <CardHeader><CardTitle className="text-lg">Logic Builder</CardTitle><CardDescription>Configure calculation rules for profiles.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Select Profile</Label>
                <Select value={selectedSectionId || ""} onValueChange={setSelectedSectionId}>
                  <SelectTrigger className="h-12 text-base font-bold"><SelectValue placeholder={loading ? "Loading..." : "Select Profile"} /></SelectTrigger>
            ...
            <CardFooter className="flex justify-between border-t p-6 bg-muted/10">
              ...
              }}><RefreshCcw className="h-3 w-3" /> Reset UI</Button>
              <Button onClick={handleSave} className="gap-2 bg-accent text-accent-foreground font-bold px-8 shadow-lg"><Save className="h-4 w-4" /> SAVE LOGIC</Button>
            </CardFooter>
          </Card>
